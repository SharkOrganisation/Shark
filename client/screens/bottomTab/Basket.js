                      ...
                      export default function Basket() {
                        ...
                        const deleteBasketItem = async (role, basketId, userId) => {
                          try {
                            ...
                            const response = await axios.delete(`http://192.168.1.14:3000/api/basket/delete/${lowercasedRole}/${basketId}/${userId}`);
                            getBasketByRole(); 
                          } catch (error) {
                            if (error.response) {
...

                            {
                              basketData.length > 0 && basketData.map((basketItem) => (
                                <View key={basketItem.id} style={styles.cardContent}>
                                  ...
                                  <View>
                                  <TouchableOpacity onPress={() => deleteBasketItem(role, basketItem.Product.id, idUser)}>
